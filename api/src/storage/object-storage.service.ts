...
export class ObjectStorageService {
  ...
  async objectExists(storageKey: string): Promise<boolean> {
    try {
      await this.headObject(storageKey);
      return true;
    } catch {
      return false;
    }
  ...
  async headObject(storageKey: string): Promise<ObjectStorageHead> {
    ...
    try {
      ...
      throw new InternalServerErrorException('Unable to read object metadata');
    }
