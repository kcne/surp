...
/**
 * Tenants for the real-database suite. Each test seeds one of its own and
 * removes it afterwards, so tests never see each other's rows.
 */
...
export function booking(seeded: Seeded, seatNumber: number, travelDate = seeded.travelDate) {
  return {
