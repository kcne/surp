...
import { Seeded, booking, removeTenant, seedTenant } from './db-fixtures';
...
describe('tenant schedule lock (real database)', () => {
  ...
  it('makes a booking wait for a schedule edit, then read what it committed', async () => {
    const edit = holdEdit((tx) =>
      tx.ride.update({ where: { id: seeded.rideId }, data: { capacity: 30 } })
    );
    await edit.acquired;
  ...
  it('refuses a confirmation whose affected set changed after it was shown', async () => {
    ...
    const again = await rides
      .update(seeded.auth, seeded.rideId, {
        capacity: 30,
        confirmationTokens: [shown.confirmationToken]
      })
      .then(
        () => {
          throw new Error('expected a second refusal');
        },
        (error: { response: { confirmationToken: string; affectedCount: number } }) =>
          error.response
      );
    ...
    await expect(
      rides.update(seeded.auth, seeded.rideId, {
        capacity: 30,
        confirmationTokens: [again.confirmationToken]
      })
