/**
 * Generated by orval v8.5.3 🍺
 * Do not edit manually.
 * SURP API
 * Backend API for SURP, providing tenant-scoped authentication, operational health endpoints, and secure session management for transportation workflows. Protected endpoints require BOTH Authorization: Bearer <accessToken> and X-Tenant-Slug headers (except /platform/* routes), and the tenant must match the token claim.
 * OpenAPI spec version: 0.1.0
 */
import type { DateRangeResponseDto } from './dateRangeResponseDto';
import type { OccupancyPointResponseDto } from './occupancyPointResponseDto';
import type { OccupancySummaryResponseDto } from './occupancySummaryResponseDto';
import type { ReportingOccupancyResponseDtoLineId } from './reportingOccupancyResponseDtoLineId';

export interface ReportingOccupancyResponseDto {
  range: DateRangeResponseDto;
  /** @nullable */
  lineId: ReportingOccupancyResponseDtoLineId;
  summary: OccupancySummaryResponseDto;
  points: OccupancyPointResponseDto[];
}
